...
# dag initialiation
@dag(
    dag_id="elt_transaction_pipeline_taskflow_api",
    start_date=datetime(2025, 8, 30),
    schedule="@daily",
    catchup=False,
    tags=["elt", "pipeline"]
)
def elt_transaction_pipeline():
    @task
    def extract_data():
        try:
            ...
            print(
                f"[INFO] Extract Data To Database {db} with schema {schema} from url {url}, Successful."
            )
    ...
    def transform_data():
        try:
            ...
            print(f"[INFO] Cleaning and Transform Dataset, Successful.")
        except Exception as e:
    ...
    def logic_business():
        try:
            ...
            df = pd.read_sql(sql, engine)
            df_sales_per_product = (
                df.groupby(["product_name", "year"])
    ...
    extract_data() >> transform_data() >> logic_business()

elt_transaction_pipeline()